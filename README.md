# WallpaperPicker-Arch
An Arch config for the wallpaper picker designed by ilyamiro

# Requirements
- quickshell
- swww
- hyprland
- imagemagick

# Notes
- Thumbnails are cached in `~/cache/wallpaper_picker/`
- Save wallpapers to ~/Images/Wallpapers

# Extras
- Added keyboard navigation to move left "A" & right "D" 
- Enter selects and closes switcher
- Designed on a 1440p monitor so I can't confirm how well it works on 1080p
- Thumbnail picker is so that images load quicker on selector

# Structure
Exactly like ilyamiro's - follow his layout

# Generate thumbnails
Run ~/wallpaper-pick/hypr/Scripts/build-thumbs.sh
Or refer to keybind.conf

If I've missed anything let me know
